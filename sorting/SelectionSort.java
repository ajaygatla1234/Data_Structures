...
public class SelectionSort {
	...
	public static void selectionSort(int arr[]) {
		...
		for(int i=0;i<arr.length-1;i++) {   //used to put the smallest elements at the correct index  
			...
			int min=i;
			for(int j=i+1;j<arr.length;j++) {  //find the smallest no. based on the index of outer loop
				if(arr[j]<arr[min]) {
					min=j;
					flag=true;
				}
			}
			if(min!=i&&flag) {         //if the minimum element is present in the same index we compare, then there is no need to swap
				int temp=arr[i];
				arr[i]=arr[min];
				arr[min]=temp;
			}
